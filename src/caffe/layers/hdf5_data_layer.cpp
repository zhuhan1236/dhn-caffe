...
void HDF5DataLayer<Dtype>::LoadHDF5FileData(const char* filename) {
  ...
  for (int i = 0; i < top_size; ++i) {
    ...
    //LOG(INFO) << "top c string " << this->layer_param_.top(i).c_str();
    /*LOG(INFO) << "top size" << top_size;*/

    hdf_blobs_[i] = shared_ptr<Blob<Dtype> >(new Blob<Dtype>());
    hdf5_load_nd_dataset(file_id, this->layer_param_.top(i).c_str(),
        MIN_DATA_DIM, MAX_DATA_DIM, hdf_blobs_[i].get());
    /*LOG(INFO) << "top i shape " << hdf_blobs_[i]->shape(0);*/
    //if ( i == 0){
        //print_gpu_matrix(hdf_blobs_[i]->cpu_data(), 10000, 1000, 20, 1000);
    //}
    //else{
        //print_gpu_matrix(hdf_blobs_[i]->cpu_data(), 10000, 21, 20, 21);
    /*}*/
