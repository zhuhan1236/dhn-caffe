...
void caffe_gpu_sum<float>(const int n, const float* x, float* y) {
  float* ones;
  CUDA_CHECK(cudaMalloc(&ones, n * sizeof(float)));
  caffe_gpu_set(n, (float)1.0, ones);
  caffe_gpu_dot(n, x, ones, y);
...
void caffe_gpu_sum<double>(const int n, const double* x, double* y) {
  double* ones;
  CUDA_CHECK(cudaMalloc(&ones, n * sizeof(double)));
  caffe_gpu_set(n, (double)1.0, ones);
  caffe_gpu_dot(n, x, ones, y);
  CUDA_CHECK(cudaFree(ones)); 
}
